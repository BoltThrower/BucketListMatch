...
public class SignUpActivity extends SherlockActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(
				this, R.array.country_list,
				android.R.layout.simple_spinner_item);

		ArrayAdapter<CharSequence> dobMonthsAdapter = ArrayAdapter
				.createFromResource(this, R.array.months,
						android.R.layout.simple_spinner_item);
		ArrayAdapter<CharSequence> dobDaysAdapter = ArrayAdapter
				.createFromResource(this, R.array.days,
						android.R.layout.simple_spinner_item);
		ArrayAdapter<CharSequence> dobYearsAdapter = ArrayAdapter
	...
	/**
	 * Checks if user given information is valid and then creates a BLM account.
	 * 
	 * @param v
	 */
	public void validateSignUp(View v) {
		// Enter code to make sure that the username is valid, does not already
		// exist, etc.
		...
		String[] userInfo = { user, pass1, first, last, dob,
				countrySpinner.getSelectedItem().toString(), email };

		// Code that makes sure that both of the passwords entered is correct.
		if (!pass1.equals(pass2)) {
			Toast.makeText(
					getApplicationContext(),
					"The passwords that were entered do not match. Please try again.",
			...
			Toast.makeText(getApplicationContext(),
					"Invalid email address was entered.", Toast.LENGTH_SHORT)
					.show();
		}

		else if (DB.addUser(userInfo) != 0) {
			Toast.makeText(getApplicationContext(),
					"Could not sign up. Please try again.", Toast.LENGTH_SHORT)
					.show();
		}

		else {
			// Add new user to the database, log the user in, and send the user
			// to the Home Screen Activity.
